...
class LoadMultimodalImages:  # for inference
    def __init__(self, thermal_path, vision_path, img_size=416, half=False):
        thermal_path = str(Path(thermal_path))  # os-agnostic
        vision_path = str(Path(vision_path))  #os-agnostic
        thermal_files = []
        if os.path.isdir(thermal_path):
            thermal_files = sorted(glob.glob(os.path.join(thermal_path, '*.*')))
        elif os.path.isfile(thermal_path):
            thermal_files = [thermal_path]

        vision_files = []
        if os.path.isdir(vision_path):
            vision_files = sorted(glob.glob(os.path.join(vision_path, '*.*')))
        elif os.path.isfile(vision_path):
            vision_files = [vision_path]

        thermal_images = [x for x in thermal_files if os.path.splitext(x)[-1].lower() in img_formats]
        thermal_videos = [x for x in thermal_files if os.path.splitext(x)[-1].lower() in vid_formats]

        vision_images = [x for x in vision_files if os.path.splitext(x)[-1].lower() in img_formats]
        vision_videos = [x for x in vision_files if os.path.splitext(x)[-1].lower() in vid_formats]
        nI, nV = len(thermal_images), len(thermal_videos) # should be the same as vision

        self.img_size = img_size
        self.thermal_files = thermal_images + thermal_videos
        self.vision_files = vision_images + vision_videos
        ...
        #else:
        self.cap = None
        assert self.nF > 0, 'No images or videos found in ' + thermal_path
    ...
    def __next__(self):
        if self.count == self.nF:
            raise StopIteration
        thermal_path, vision_path = self.thermal_files[self.count], self.vision_files[self.count]
        ...
        #     print('video %g/%g (%g/%g) %s: ' % (self.count + 1, self.nF, self.frame, self.nframes, path), end='')
            ...
            # Read image
        self.count += 1
        img0_thermal = cv2.imread(thermal_path)  # BGR
        img0_vision = cv2.imread(vision_path)
        assert img0_vision is not None and img0_thermal is not None, 'Image Not Found ' + thermal_path
        print('image %g/%g %s %s: ' % (self.count, self.nF, thermal_path,vision_path), end='')



        # Padded resize
        img_thermal = letterbox(img0_thermal, new_shape=self.img_size)[0]
        img_vision = letterbox(img0_vision, new_shape=self.img_size)[0]

        # Convert
        img_thermal = img_thermal[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        img_vision = img_vision[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        img_thermal = np.ascontiguousarray(img_thermal, dtype=np.float16 if self.half else np.float32)  # uint8 to fp16/fp32
        img_vision = np.ascontiguousarray(img_vision, dtype=np.float16 if self.half else np.float32)  # uint8 to fp16/fp32
        img_thermal /= 255.0  # 0 - 255 to 0.0 - 1.0
        img_vision /= 255.0
        # cv2.imwrite(path + '.letterbox.jpg', 255 * img.transpose((1, 2, 0))[:, :, ::-1])  # save letterbox image
        return thermal_path,vision_path, img_thermal, img0_thermal, img_vision, img0_vision, self.cap
...
class LoadMultimodalImagesAndLabels(Dataset):
    def __init__(self, thermal_path, vision_path, img_size=416, half=False):
        ...
        self.thermal_files = []
        if os.path.isdir(thermal_path):
            self.thermal_files = sorted(glob.glob(os.path.join(thermal_path, '*.*')))
        elif os.path.isfile(thermal_path):
            self.thermal_files = [thermal_path]

        self.vision_files = []
        if os.path.isdir(vision_path):
            self.vision_files = sorted(glob.glob(os.path.join(vision_path, '*.*')))
        elif os.path.isfile(vision_path):
            self.vision_files = [vision_path]

        n = len(self.thermal_files)   # should be the same as vision
...
class LoadFourChannelImages:  # for inference
    def __init__(self, path, img_size=416, half=False):
        path = str(Path(path))  # os-agnostic
        files = []
        if os.path.isdir(path):
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        if self.rect:
            ...
            try:
                with open(sp, 'r') as f:  # read existing shapefilep
...
class LoadFourChannelImagesandLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_labels=False, cache_images=False):
        path = str(Path(path))  # os-agnostic
        with open(path, 'r') as f:
            ...
            self.thermal_img_files = [x.replace('visible', 'lwir') for x in self.img_files]
        ...
        if cache_images:  # if training
            ...
            pbar = tqdm(range(len(self.img_files)), desc='Caching images')
            for i in pbar:  # max 10k images
                self.imgs[i] = load_image_four_channel(self, i)
                gb += self.imgs[i].nbytes
                pbar.desc = 'Caching images (%.1fGB)' % (gb / 1E9)
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            # Load mosaic
            img, labels = load_mosaic_four_channel(self, index)
            h, w = img.shape[:2]
            ratio, pad = None, None

        else:
            # Load image
            img = load_image_four_channel(self, index)
...
def load_image_four_channel(self, index):
    # loads 1 image from dataset
    img = self.imgs[index]
    if img is None:
        img_path = self.img_files[index]
        thermal_img_path = self.thermal_img_files[index]
        img = cv2.imread(img_path, cv2.IMREAD_UNCHANGED)  # BGR
        thermal_img = cv2.imread(thermal_img_path, 0)
        assert img is not None, 'Image Not Found ' + img_path
        assert thermal_img is not None, 'Thermal Image Not Found' + thermal_img_path
        thermal_img = np.expand_dims(thermal_img,2)
        img = np.concatenate((img, thermal_img), 2)
        r = self.img_size / max(img.shape)  # resize image to img_size
        if self.augment and (r != 1):  # always resize down, only resize up if training with augmentation
            h, w = img.shape[:2]
            return cv2.resize(img, (int(w * r), int(h * r)), interpolation=cv2.INTER_LINEAR)  # _LINEAR fastest
...
def load_mosaic_four_channel(self, index):
    # loads images in a mosaic
    labels4 = []
    s = self.img_size
    xc, yc = [int(random.uniform(s * 0.5, s * 1.5)) for _ in range(2)]  # mosaic center x, y
    ...
    for i, index in enumerate(indices):
        ...
        # place img in img4
        if i == 0:  # top left
            img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tilesl
            x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
            x1b, y1b, x2b, y2b = w - (x2a - x1a), h - (y2a - y1a), w, h  # xmin, ymin, xmax, ymax (small image)
        elif i == 1:  # top right
            x1a, y1a, x2a, y2a = xc, max(yc - h, 0), min(xc + w, s * 2), yc
